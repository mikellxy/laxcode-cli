...
- `Session` holds the latest `RequestContext`. Each appended message and successful compaction checkpoints `request_context.json`; originals remain in append-only `history.jsonl`. Startup restores the snapshot, recovering interrupted commits from a pending record without duplicating history. Sessions without a snapshot migrate once from legacy history, `sys_message.json`, and `meta.json`.
- Non-system messages have a monotonically increasing session-local `Seq`. Each assistant response gets a `TurnID`, inherited by its tool results together with `ToolCallGroupID`. Ordinary assistant messages have no tool group; user/system messages have no `TurnID`.
