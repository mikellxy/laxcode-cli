...
// captureStdout 捕获 fn 执行期间写往 os.Stdout 的内容，用于校验
// LoadSkills 对非法技能“静默跳过、绝不阻断”的行为契约：当前实现不
// 输出任何警告，未来若补回警告落点，测试仍可据此守住基线。
...
func TestLoadSkillsInvalidSkillSilentlySkipped(t *testing.T) {
	root := t.TempDir()
	writeSkillFile(t, root, "bad", "---\nname: bad\n---\n")
	writeSkillFile(t, root, "good", validSkillMD("good", "好技能"))
	...
	if out != "" {
		t.Errorf("非法技能应静默跳过、不产生任何输出，实际输出: %q", out)
	}
}
