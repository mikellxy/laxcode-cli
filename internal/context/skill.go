// Package context 负责 agent 运行时上下文的动态组装。
//
// 当前提供技能（skill）索引能力：在 agent 启动时发现并校验工作目录下的
...
// skills 目录不存在时静默返回空集合；任何非法文件仅输出警告并被跳过，
// 绝不阻断启动。
func LoadSkills(workDir string) []Skill {
	...
	for _, dirEntry := range dirEntries {
		if !dirEntry.IsDir() {
			continue // skills/ 下的散置文件直接忽略，不产生警告
		}
...
func loadSkillDir(dirPath, dirName string) (Skill, bool) {
	...
	if err != nil {
		warnSkipSkill(dirName, "技能目录不可读: %v", err)
		return Skill{}, false
	}
	...
	if err != nil {
		warnSkipSkill(dirName, "读取 SKILL.md 失败: %v", err)
		return Skill{}, false
	}
	return parseSkill(string(content), dirName)
}

// parseSkill 执行校验管线，顺序即警告优先级：
// ① frontmatter 边界 ② YAML 解析 ③ 必填字段非空白
// ④ name 与目录名一致 ⑤ name 符合命名规范；任一步失败即跳过并警告。
func parseSkill(content, dirName string) (Skill, bool) {
	fmText, failReason := extractFrontmatter(content)
	if failReason != "" {
		warnSkipSkill(dirName, "%s", failReason)
		return Skill{}, false
	}

	var fm frontmatterYAML
	if err := yaml.Unmarshal([]byte(fmText), &fm); err != nil {
		warnSkipSkill(dirName, "frontmatter 不是合法 YAML: %v", err)
		return Skill{}, false
	}

	if strings.TrimSpace(fm.Name) == "" || strings.TrimSpace(fm.Description) == "" {
		warnSkipSkill(dirName, "frontmatter 的 name 与 description 均为必填且不能为空白")
		return Skill{}, false
	}

	if fm.Name != dirName {
		warnSkipSkill(dirName, "frontmatter name %q 与所在目录名不一致，请将 name 改为 %q 或将目录重命名为 %q",
			fm.Name, dirName, fm.Name)
		return Skill{}, false
	}

	if len(fm.Name) > skillNameMaxLen || !skillNamePattern.MatchString(fm.Name) {
		warnSkipSkill(dirName, "name %q 不符合命名规范（仅小写字母与数字、连字符作分隔，如 pdf-tools，长度不超过 %d），请将目录与 name 一并重命名",
			fm.Name, skillNameMaxLen)
		return Skill{}, false
	}
