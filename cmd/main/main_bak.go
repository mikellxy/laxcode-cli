...
// main_bak.go 是迁移前的老前端入口（engine.TerminalLoop 交互 / engine.OneShotLoop
// one-shot），已由 main.go + cmd/run_cli（交互）+ cmd/run_oneshot（one-shot）取代。
// 加 //go:build ignore 将其排除出常规构建：一是避免与 main.go 的 func main()
// 重复声明，二是断开其对老 internal/provider → anthropic SDK 的依赖（该依赖
// 缺 go.sum 条目，会连累 make build）。文件仅作迁移参考，不参与编译。
package main
